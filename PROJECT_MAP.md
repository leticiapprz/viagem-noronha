...
- `pullFromFirebase()` — na carga da página, busca o banco inteiro (`FB + '/.json'`) e sobrescreve o `localStorage` com o que vier de `gastos`, `diadia`, `checklist-v2`, `gastosCustom`, `compras` e `resolver`. É a **única** fonte de carga inicial de dados — não existe mais nenhum "push" automático de defaults (ver bug crítico abaixo).
